...
const DeductionModal = ({ onClose, onAdd, participants, totalAmount }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-4/5 rounded-2xl border border-[#202020] bg-[#F8F7F4] p-0 font-['ONE_Mobile','SF_Pro','sans-serif'] shadow-lg sm:w-[380px]">
        {/* 상단 타이틀/닫기 */}
        <div className="relative mb-4 flex items-center justify-between px-4 pb-2 pt-6 font-['ONE-Mobile-Title']">
          ...
          <button
            onClick={onClose}
            className="rounded px-2 py-1 text-base font-bold transition-colors hover:text-red-400"
          >
            취소
          </button>
          <button
        ...
        <div className="flex h-fit max-h-[70vh] flex-col gap-4 overflow-scroll px-6 pb-6">
          ...
          <div className="flex flex-col items-start gap-1">
            ...
            <input
              ...
              placeholder="예: 술값 등"
              className="w-full rounded-lg border border-[#D3D3D3] bg-white px-4 py-3 text-base text-[#202020] placeholder:text-[#7C7C7C] focus:outline-none"
              maxLength={20}
            />
          </div>
          {/* 금액 입력 */}
          <div className="flex flex-col items-start gap-1">
            ...
            <div className="relative flex w-full items-center">
              <span className="absolute left-4 text-base text-[#202020]">
                ₩
              </span>
              <input
                type="text"
                ...
                placeholder="여기에 금액을 입력하세요..."
                className="w-full rounded-lg border border-[#D3D3D3] bg-white py-3 pl-10 pr-4 text-right text-base text-[#202020] placeholder:text-[#7C7C7C] focus:outline-none"
                min={0}
              />
            </div>
          </div>
              ...
              {participants.map((participant, index) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between rounded-lg border px-4 py-3 font-['ONE-Mobile-Title'] text-[16px] shadow-sm transition ${selected ? 'border-[#4DB8A9] bg-[#4DB8A9] text-white' : 'border-[#E5E4E0] bg-white text-[#202020] hover:bg-[#4DB8A9]/10'}`}
                  >
                    <span
